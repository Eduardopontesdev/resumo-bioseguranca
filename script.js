// 30 perguntas de nível superior sobre Biossegurança, NR-32 e Resíduos
const perguntas = [
    ...
    { texto: "A cor que representa o Risco Biológico no Mapa de Riscos, conforme padronização, é:", alternativas: ["Amarelo", "Verde", "Vermelho", "Laranja"], correta: 2 },
    { texto: "Qual método de extinção de incêndio atua pela remoção do oxigênio (comburente), isolando o fogo?", alternativas: ["Resfriamento (uso de água)", "Abafamento (uso de espuma, CO₂ ou manta)", "Remoção do combustível", "Inibição química"], correta: 1 },
    ...
    { texto: "Qual dos seguintes resíduos pertence ao Grupo A (Infectante), subgrupo A1 (alta patogenicidade), exigindo tratamento prévio como autoclavação?", alternativas: ["Carcaça de animal de experimentação", "Peça anatômica humana (membro amputado)", "Cultura de laboratório de Mycobacterium tuberculosis", "Seringa com agulha (perfurocortante)"], correta: 2 },
    ...
    { texto: "Para incêndio envolvendo líquidos inflamáveis como álcool e gasolina (Classe B), qual o método de extinção mais eficaz?", alternativas: ["Jogar água em abundância (resfriamento)", "Usar extintor de água pressurizada", "Usar extintor de espuma, CO₂ ou pó químico (abafamento)", "Abrir janelas para o fogo sair"], correta: 2 },
...
function verificarQuizCompleto() {
    ...
    if (todasRespondidas) {
        ...
        if (percentual >= 70) {
            ...
            mensagem = `📚 BOM! Você acertou ${acertos}/30 (${percentual.toFixed(1)}%). Revise os tópicos de resíduos e NR-32 e tente novamente para alcançar nota máxima.`;
            cor = '#fff3cd';
        } else {
            mensagem = `⚠️ ATENÇÃO: Você acertou ${acertos}/30 (${percentual.toFixed(1)}%). Estude o resumo com calma, foque nos grupos de resíduos e classes de risco, depois refaça o quiz.`;
            cor = '#fce4e4';
        }
